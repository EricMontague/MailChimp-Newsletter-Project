...
def test_create_new_artist_with_valid_data(flask_test_client, auth, user, json, db):
    ...
    response = flask_test_client.post(
        "/api/v1/artists", headers=get_headers(token), data=json.dumps(artist_object)
    )
...
def test_create_new_artist_with_invalid_data_must_fail(
    ...
    token = auth.register(user.username, "password", user.email)
    response = flask_test_client.post(
        "/api/v1/artists", headers=get_headers(token), data=json.dumps(test_input)
    )
...
def test_create_new_artist_with_duplicate_name_must_fail(
    flask_test_client, auth, user, artist, json
):
...
def test_update_artist_info_with_valid_data(
    flask_test_client, auth, user, db, artist, json
):
...
def test_update_artist_info_with_invalid_data_must_fail(
    flask_test_client, auth, user, db, json, test_input, artist, expected
):
...
def test_update_artist_info_with_missing_field_must_fail(
    flask_test_client, auth, user, db, json, test_input, artist, expected
):
...
def test_update_artist_id_must_fail(flask_test_client, auth, user, json, artist):
    """Test that an artist's id cannot be updated."""
    updated_artist_object = {
        "id": 1000,
        "name": artist.name,
        "bio": artist.bio,
...
def test_delete_artist(flask_test_client, auth, user, artist, db):
    ...
    response = flask_test_client.delete(
        f"/api/v1/artists/{artist.id}", headers=get_headers(token)
    )
    assert response.status == "204 NO CONTENT"
    assert response.content_type == "application/json"
    assert response.get_data(as_text=True) == ""
...
def test_delete_artist_not_found_must_fail(flask_test_client, auth, user):
    ...
    token = auth.register(user.username, "password", user.email)
    response = flask_test_client.delete(
        "/api/v1/artists/100", headers=get_headers(token)
    )
