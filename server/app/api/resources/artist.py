...
class ArtistAPI(Resource):
    ...
    def get(self, artist_id):
        ...
        return self.schema.dump(artist), HTTPStatus.OK

    def put(self, artist_id):
        """Update a single artist resource."""
        json_data = request.get_json()
        try:
            updated_artist = self.schema.load(json_data)
...
class ArtistListAPI(Resource):
    ...
    def get(self):
        ...
        query = Artist.query
        return paginate(Artist.__tablename__, query, self.schema), HTTPStatus.OK
        
    def post(self):
        """Create a new artist resource."""
        json_data = request.get_json()
        try:
            new_artist = self.schema.load(json_data)
        except ValidationError as err:
            return {"message": err.messages}, HTTPStatus.BAD_REQUEST
        ...
        db.session.commit()
        return self.schema.dump(new_artist), HTTPStatus.CREATED
